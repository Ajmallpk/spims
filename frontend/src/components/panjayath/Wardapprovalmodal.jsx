...
function DetailRow({ label, value, mono = false }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3 py-2.5 border-b border-slate-100 last:border-0">
      ...
      <span
        className={`text-sm text-slate-800 font-medium flex-1 break-words ${
          mono ? "font-mono text-xs bg-slate-50 px-2 py-1 rounded-lg" : ""
        }`}
      >
...
export default function WardApprovalModal({ ward, onClose, onSuccess }) {
  ...
  const documents = ward.documents || ward.uploaded_documents || [];
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-1 divide-y divide-slate-100">
            <DetailRow label="Ward Name" value={ward.ward_name || ward.name} />
            <DetailRow label="Email" value={ward.email} />
            <DetailRow label="Phone" value={ward.phone || ward.phone_number} />
            <DetailRow
              label="Address"
              value={ward.address || ward.ward_address}
            />
            <DetailRow
              label="Submitted"
              value={formatDate(ward.submitted_at || ward.created_at)}
            />

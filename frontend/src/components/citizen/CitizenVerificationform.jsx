...
const WARD_OPTIONS = Array.from({ length: 20 }, (_, i) => ({
  value: `ward-${String(i + 1).padStart(2, "0")}`,
...
const CitizenVerificationForm = ({
  ...
  const validate = () => {
    ...
    if (!form.wardName) newErrors.wardName = "Please select a ward";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (files.aadhaar) formData.append("aadhaarImage", files.aadhaar);
      if (files.selfie) formData.append("selfieImage", files.selfie);

      const response = await fetch("/api/citizen/submit-verification/", {
        method: "POST",
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-6 space-y-6">
      ...
      <section className="space-y-4">
        ...
        <div id="field-wardName">
          <InputField label="Ward Name" required error={errors.wardName}>
            <select
              name="wardName"
              value={form.wardName}
              onChange={handleChange}
              className={inputClass(!!errors.wardName)}
            >

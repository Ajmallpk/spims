...
const LoginForm = () => {
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      {/* Error Banner */}
      {error && (
        <div className="flex items-start gap-3 px-4 py-3.5 rounded-xl bg-red-50 border border-red-100 text-red-700 text-sm">
          <AlertCircle
            size={17}
            className="flex-shrink-0 mt-0.5 text-red-500"
            strokeWidth={2}
      ...
      <LoginInput
        ...
        rightElement={
          <button
            ...
            onClick={() => setShowPassword((v) => !v)}
            disabled={loading}
            className="text-slate-400 hover:text-emerald-600 transition-colors duration-150 disabled:cursor-not-allowed"
            aria-label={showPassword ? "Hide password" : "Show password"}
          >

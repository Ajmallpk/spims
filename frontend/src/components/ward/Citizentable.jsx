...
//               {["Citizen Name", "Email", "Phone", "Joined Date", "Status", "Action"].map((col, i) => (
//                 <th
//                   key={i}
    ...
    <div className="bg-white rounded-2xl shadow-md border border-gray-100 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-100">
              {["Citizen Name", "Email", "Phone", "Joined Date", "Action"].map((col, i) => (
                <th
                  key={i}
          ...
          <tbody className="divide-y divide-gray-50">
            {isLoading ? (
              ...
              citizens.map((citizen) => (
                <tr
                  key={citizen.id}
                  className="hover:bg-teal-50/40 transition-colors duration-100 group"
                >
                  {/* Name */}
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                  ...
                  <td className="px-5 py-4 text-right">
                    <button
                      onClick={() => onView(citizen.id)}
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold
                        text-teal-600 bg-teal-50 hover:bg-teal-100 border border-teal-100 rounded-lg
                        transition-all duration-150 group-hover:shadow-sm"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
